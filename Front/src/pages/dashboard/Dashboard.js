...
export default function Dashboard(props) {
  ...
  const postFileBisnes = async (data) => {
    setIsLoadingGrafik(true)
    await fetch(process.env.REACT_APP_API + 'plan/upload_excel', {
      headers: {
        'Authorization': "Token " + localStorage.getItem('id_token')
      },
      method: "POST",
  ...
  const saveAsPdf = (e) => {
    ...
    html2canvas(document.querySelector("#cardFilter")).then(canvas => {
      ...
      pdf.text('Период (дата по первому замеру):', 10, 30);
      pdf.text('Период (дата по последнему замеру):', 10, 40);
      pdf.text('Ствол (по первому стволу):', 10, 50);
      pdf.text('Скважинные операции:', 10, 60);
      pdf.setFontSize(10)
      pdf.text(filterDataBisnes.accompaniment_type.length > 0 ? filterDataBisnes.accompaniment_type.join(',') : 'Круглосуточно, ТЗ', 60, 20);
      pdf.text(filterDataBisnes.data_start ? filterDataBisnes.data_start.split('-').reverse().join('.') : '-', 120, 30);
      pdf.text(filterDataBisnes.data_end ? filterDataBisnes.data_end.split('-').reverse().join('.') : '-', 130, 40);
      pdf.text(filterDataBisnes.wellbore ? filterDataBisnes.wellbore : '-', 100, 50);
      pdf.text(filterDataBisnes.well_operation > 0 ? 'Включены' : 'Отключены', 90, 60);
